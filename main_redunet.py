...
def main_worker(args):
    ...
    # train_dataset = datasets.MNIST(traindir, transforms.Compose([
    #         transforms.ToTensor(),
    #         transforms.Normalize(mean=(0.4914, 0.4822, 0.4465), std=(0.2023, 0.1994, 0.2010)),
    # ]))

    # val_dataset = datasets.MNIST(traindir, transforms.Compose([
    #         transforms.ToTensor(),
    #         transforms.Normalize(mean=(0.4914, 0.4822, 0.4465), std=(0.2023, 0.1994, 0.2010)),
    # ]))
    ...
    if args.arch in model_names:
        ...
        if args.scattering_wph:
            ...
            scattering = ScatteringTorch2D_wph(J=J, shape=(28, 28), L=L_ang, A=A, max_order=max_order,
                                               backend=args.backend)
        else:
            scattering = Scattering2D(J=J, shape=(28, 28), L=L_ang, max_order=max_order,
                                      backend=args.backend)
        # Flatten scattering
        scattering = nn.Sequential(scattering, nn.Flatten(1, 2))


        X_train = torch.from_numpy(np.load('MNIST-X_train.npy'))
        X_test = torch.from_numpy(np.load('MNIST-X_test.npy'))
        scat_X_train = scattering(X_train)
        scat_X_test = scattering(X_test)
        save_dir = f'./wph{args.scattering_wph}_J{args.scattering_J}_L-ang{args.scat_angles}'
        os.makedirs(save_dir, exist_ok=True)
        np.save(os.path.join(save_dir, 'MNIST-scat_X_train.npy'), scat_X_train)
        np.save(os.path.join(save_dir, 'MNIST-scat_X_test.npy'), scat_X_test)
        print(scat_X_train.shape, scat_X_test.shape)
        exit()
